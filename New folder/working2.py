...
@bot.callback_query_handler(func=lambda call: True)
def  button_click1(call):
        print(f"Received callback data: {call.data}")  # Debugging statement
        ...
        if call.data.startswith("pay_"):
            ...
            print(amount)  # Debugging statement
    ...
    # if call.data.startswith("pay_"):
    #     data_parts = call.data.split("_")
    #     if len(data_parts) >= 2:  # Check if the list has at least two items
    #         item_name = data_parts[1] 
    #         size = data_parts[2]
    #         # ... Do other things with item_name
    #     else: 
    #         print("Error: Not enough information!")


    #     if item_name is not None and size is not None:
    #         amount = menu_items.get(item_name, {}).get("size", {}).get(size)
    #         if amount is not None:
    #             # Implement payment logic here
    #             bot.reply_to(call.message, f"Payment processed for {item_name} - {size}. Amount: {amount}")
    ...
    #             print(3)
    #     else:
    #         bot.reply_to(call.message, "Error: Invalid data format.")
    #         print(2)
    # else:
    #     data_parts = call.data.split("_")
    #     bot.reply_to(call.message, "Error: unknow!")
    #     if len(data_parts) >= 2:
    #         item_name = data_parts[1]
    #     else:
    #         print("Error: Not enough information in the list!")
    #     amount = menu_items.get(item_name, {}).get("size", {}).get(size)
    #     print(amount,menu_items)


def display_payment_options2(message, item_name):
    item = menu_items2.get(item_name)
    if item:
        prices = item["size"]
        markup = types.InlineKeyboardMarkup()
        for size, price in prices.items():
...
def display_payment_options3(message, item_name):
